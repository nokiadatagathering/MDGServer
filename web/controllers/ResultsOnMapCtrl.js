define(function () {
  'use strict';
  return function ($scope, $rootScope, $http, $stateParams, chartManager, nokia_app_id, nokia_app_code) {
    var
      mapContainer,
      markers = [],
      map = {},
    ...
    if ($rootScope.selectedResults.length !== 0) {
      ...
      if (localStorage.getItem('selectedResults')) {
        ...
        $rootScope.goState('page.results', { surveyId: $stateParams.surveyId });
      }
    }

    $scope.getMapData = function () {
      chartManager.getChartData({ survey: $stateParams.surveyId, results: $scope.selectedResults }).then(
