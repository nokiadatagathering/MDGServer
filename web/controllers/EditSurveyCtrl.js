define(function () {
  'use strict';
  return function ($scope, $rootScope, $http, $location, $window, $stateParams, surveysManager) {
    ...
    $scope.getSurveyData = function (surveyId) {
      ...
      if (!surveyId) {
        ...
        surveysManager.surveyInfo(surveyId).then(
        ...
        return surveysManager.editSurvey({ id: $stateParams.surveyId, body: clearSurveyData(survey) }).then(
          ...
          function failed (err) {
            if (err.status === 409) {
              ...
              $rootScope.goState('page.editsurvey', { surveyId: $stateParams.surveyId });
            }
