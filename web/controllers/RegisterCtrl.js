define(function () {
  'use strict';
  return function ($scope, $http, $location, $window, $stateParams, $rootScope, $base64,
                   profileManager, errorsManager, validateManager, countries) {
    $scope.user = {};
    $scope.countries = countries;
    $scope.errors = {};

    $scope.signUp = function () {
      if ($scope.registerForm.$error.required.length > 0) {
        _.each($scope.registerForm.$error.required, function (field) {
          $scope.registerForm[field].$error.required = true;
        });

        $('input.ng-pristine').addClass('ng-dirty').removeClass('ng-pristine');
      } else if ($('.error-message:not(.ng-hide)').length === 0) {
        $scope.update();

        profileManager.registration($scope.newUser).then(
          function success (config) {
            $rootScope.goState('pageGetStarted.register.successRegistration');
          },

          function failed (err) {
            if (err.status === 400 && _.find(err.data, function (data) {
              return data.path === 'email';
            })) {
              $scope.errorEmailPattern = true;
            }

            console.log("error:", err);
          });
      }
    };

    $scope.update = function () {
      $scope.newUser = angular.copy($scope.user);

      if ($scope.newUser.password) {
        $scope.newUser.password = $base64.encode($scope.newUser.password);
      }
    };

    $scope.checkExistingUsername = function () {
      validateManager.validate({ field: 'username', value: $scope.user.username }).then(
        function success () {
          $scope.errorExistingUsername = false;
        },

        function failed () {
          $scope.errorExistingUsername = true;
        }
      );
    };

    $scope.$watch('registerForm.username.$error.pattern', function () {
      $scope.errorUsernamePattern = $scope.registerForm.username.$error.pattern;
    });

    $scope.addFieldsErrors = function () {
      var fields = {
        firstName: [{ minlength: 2 }, { maxlength: 60 }, 'required'],
        lastName: [{ minlength: 2 }, { maxlength: 60 }, 'required'],
        username: [{ minlength: 5 }, { maxlength: 13 }, 'required', 'errorUsernamePattern', 'errorExistingUsername'],
        phone: ['errorPhoneMaxLength', 'errorPhoneMinLength', 'required'],
        email: [{ minlength: 3 }, { maxlength: 60 }, 'required', 'email', 'errorEmailPattern'],
        password: [{ minlength: 8 }, { maxlength: 20 }, 'required'],
        company: [{ minlength: 3 }, { maxlength: 60 }, 'required'],
        country: ['required'],
        industry: [{ minlength: 3 }, { maxlength: 60 }, 'required']
      };

      _.map(_.pairs(fields), function (field) {
        $scope.errors[field[0]] = errorsManager.getFieldErrorsHtml(field[0], field[1], 'registerForm');
      });
    };

    $scope.addFieldsErrors();
  };
});
